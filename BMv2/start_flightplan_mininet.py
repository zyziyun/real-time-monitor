...
class FPTopo(Topo):
    ...
    def __init__(self, host_spec, switch_spec, sw_path, log, verbose, cfg_file, bw, qlen):
        ...
        self.switch_prog_num = defaultdict(int)
    ...
    def run_switch_programs(self, net):
        for name, opts in self.all_switches.items():
            for program in opts['programs']:
                i = self.switch_prog_num[name]
                self.switch_prog_num[name] += 1
                if isinstance(program, str):
                    cmd = program
                    fg = False
                else:
                    raise Exception("Items under 'programs' for a switch must be strings.")
